...
trait InstantiableNPDT[Mapping[_ <: Target] <: NumericPDT.NumericTypes, T]
  ...
  def fromMinima(value: MappingMinima[Mapping, SupportedTargets, Double])(using target: Target): T

object InstantiableNPDT:
  @nowarn(
    "msg=New anonymous class definition will be duplicated at each inline site"
  )
  inline def derive[Mapping[_ <: Target] <: NumericPDT.NumericTypes, P <: PDT[
    Mapping
  ]] =
    new InstantiableNPDT[Mapping, P]:
      val minimaNum = summonInline[Numeric[MappingMinima[Mapping, SupportedTargets, Double]]]
